...
- ==IoC（Inversion of Control）控制反转==
  ...
  将创建对象的权利交给外部，由外部提供我们需要使用的对象。此思想称为控制反转。
...
#### 2.4 实现FactoryBean\<T>方式【属于上面方法三的变种，需掌握】
...
public class UserDaoFactoryBean implements FactoryBean<UserDao> {
    ...
    /*可以通过该方法控制是否单例，默认是单例，如果return false;则不是单例
...
​	getBean时会执行init方法，但关闭容器不能执行destory方法
...
- 初始化容器(new ClassPathXmlApplicationContext("applicationContext.xml");)
  1. 创建对象，进行内存分配
  2. 执行构造方法
  3. 执行属性注入(set操作)
  4. 执行bean的初始化方法 init/afterPropertiesSet
- 使用bean
  1. 执行业务
- 关闭/销毁容器
  1. 执行bean的销毁方法（destroy）
...
2. **要有对应的set方法**（后面学了注解就不需要set方法了，暴力反射）
3. 使用按类型装配时（byType）必须保障容器中相同类型的bean唯一，推荐使用，且按类型装配对应bean的id和name都可以不要
4. 使用按名称装配时（byName）必须保障容器中具有与属性名称相同名称的bean，因变量名与配置耦合，不推荐使用
5. 自动装配优先级低于setter注入与构造器注入，同时出现时自动装配配置失效
