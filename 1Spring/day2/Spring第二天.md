...
**【第二步】在applicationContext.xml中开启context命名空间，加载jdbc.properties属性文件**
...
<!-- 说明：idea自动识别${}加载的属性值，需要手工点击才可以看到像下面这样的原始书写格式-->
...
- ClassPathXmlApplicationContext也能实现上面的延迟加载的功能，只不过要在每一个Bean中添加 `lazy-init`属性，并把值设为true
...
问题2：@Component注解和@Controller、、@Service、@Repository三个衍生注解有什么区别？(后面三个衍生可以增强可读性，功能上没区别)
...
@Configuration
@ComponentScan("com.itheima")
//@Import:导入配置信息(@Import只能写一次，如果有多个，要导入的配置类要用","隔开添加)
...
```java
@Configuration
@ComponentScan({"com.itheima.config","com.itheima.service","com.itheima.dao"})  //只要com.itheima.config包扫到了就行，三个包可以合并写成com.itheima    注意：要在配置类上添加@Component
public class SpringConfig {
}
```
