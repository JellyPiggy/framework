...
- 作用：在不惊动原始设计的基础上为其进行功能增强。即：**在不改变方法源代码的基础上已有方法进行功能增强。**
- 实现原理：动态代理技术
...
- 连接点（JoinPoint）：除构造方法以外的方法。例如：update()、delete()、select()等都是连接点。
- 切(入)点（Pointcut）：进行了功能增强的方法
...
public class MyAdvice {
    ...
    @Pointcut("execution(void com.jide.dao.AccountDao.find*())")
    private void pt(){}
    ...
    //如果某切入点被多个通知用到像上面这样会更方便，如果只有一个用了则可以像下面这样用
    @Around("execution(void com.jide.dao.AccountDao.update())")
    public void method3(){
        System.out.println(System.currentTimeMillis());
    }
...
//开启注解开发AOP功能，如果用配置文件的话要添加<aop:aspectj-autoproxy/> aop注解支持
...
```
execution（public * com.itheima.service.*.findById(*))
```

- 参数中用 * 表示任意类型一个参数，用 .. 表示任意类型任意个，参数列表如果是基本数据类型则直接用，如果是引用类型要用  全类名
...
- 返回值类型对于**增删改类使用精准类型**加速匹配，对于**查询类使用\*通配**快速描述
...
- AOP通知共分为5种类型
  ...
  - 正常返回通知(了解)：在切入点方法执行之后执行，如果切入点方法内部出现异常将不会执行。
  - 抛出异常后通知(了解)：在切入点方法执行之后执行，只有当切入点方法内部出现异常之后才执行。
  
  (不用记)通知的执行顺序：
  
  ​		无异常：前置通知 ---> 环绕前 --->  切入点方法 ---> 正常返回  --->  环绕后 --->  后置通知
  
  ​						环绕前 --->  前置通知 ---> 切入点方法 ---> 正常返回  --->  后置通知 --->  环绕后
  
  ​		有异常： 前置通知 ---> 环绕前 --->  切入点方法异常前部分--->  抛出异常通知 --->  后置通知
  
  ​						环绕前 --->  前置通知 ---> 切入点方法异常前部分--->  抛出异常通知 --->  后置通知
...
//异常通知,可以添加Exception参数，就会吧异常信息封装到异常对象中，你可以在此处理
@AfterThrowing(value = "pt()", throwing = "ex")
public void myAfterThrowing(Exception ex) {
    System.out.println("异常通知");
    System.err.println(ex.getMessage());  //System.err.println打印出来是红色字体
}
```
...
1. Spring注解式事务**通常添加在业务层接口中而不会添加到业务层实现类中，降低耦合**
2. 注解式事务可以添加到业务方法上表示当前方法开启事务，也可以**添加到接口上表示当前接口所有方法开启事务**
...
说明：==对于RuntimeException类型异常或者Error错误，Spring事务能够进行回滚操作。但是对于编译器异常，Spring事务是不进行回滚的，所以需要使用rollbackFor来添加其它要回滚的异常。==
